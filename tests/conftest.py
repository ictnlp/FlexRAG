...
@pytest.fixture
def mock_llamacpp_client(mocker):
    """Mock llama_cpp.Llama for testing LlamacppGenerator without actual model files."""

    # Mock the Llama class
    mock_llama_instance = mocker.MagicMock()

    # Mock create_completion method
    def mock_create_completion(prompt, **kwargs):
        # Generate different responses based on prompt content
        if "Bruce Wayne" in prompt:
            content = "Bruce Wayne is a fictional character"
        elif "Thomas Wayne" in prompt:
            content = "Thomas Wayne is the father of Bruce Wayne"
        elif "capital of China" in prompt:
            content = "Beijing"
        else:
        ...
        if stop_strings:
            for stop_str in stop_strings:
                if stop_str in content:
                    content = content.split(stop_str)[0] + stop_str
                    break

        return {"choices": [{"text": content}]}

    # Mock create_chat_completion method
    def mock_create_chat_completion(messages, **kwargs):
        ...
        if "Bruce Wayne" in user_message:
            content = "Bruce Wayne is a fictional character"
        elif "Thomas Wayne" in user_message:
            content = "Thomas Wayne is the father of Bruce Wayne"
        elif "capital of China" in user_message:
            content = "Beijing"
        else:
            content = "This is a mock response"
    ...
    mock_llama_instance.create_completion = mock_create_completion
    mock_llama_instance.create_chat_completion = mock_create_chat_completion

    # Mock the entire llama_cpp module to handle lazy import
    mock_llamacpp_module = mocker.MagicMock()
    mock_llamacpp_module.Llama.return_value = mock_llama_instance

    # Patch the module import itself
    mocker.patch.dict("sys.modules", {"llama_cpp": mock_llamacpp_module})
