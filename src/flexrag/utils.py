...
class Register(Generic[RegistedType]):
    ...
    def load(
        self,
        config: DictConfig,
        allow_load_from_repo: bool = False,
        **kwargs,
        ...
        :param allow_load_from_repo: Whether to allow loading the item from a FlexRAG repository.
            Defaults to False.
        :type allow_load_from_repo: bool
        ...
        def load_item(type_str: str) -> RegistedType:
            ...
            if allow_load_from_repo:
                ...
                if type_str.count("/") <= 1:
                    try:
                        ...
                        dir_name = os.path.join(
                            FLEXRAG_CACHE_DIR,
                            f"{repo_url.split('/')[0]}--{repo_url.split('/')[1]}",
                        )
                ...
                # load the item from the local repository
                    ...
                    # prepare the cls
                    id_path = os.path.join(snapshot, "cls.id")
                    ...
                    # load the item
                    return self[cls_name]["item"].load_from_local(snapshot, new_cfg)
