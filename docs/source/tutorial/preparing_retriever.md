...
Retriever is one of the most important component in the RAG pipeline. It retrieves the top-k relevant contexts from the knowledge base for a given query. In FlexRAG, retrievers are knowledge-base backed components such as `APIBasedRetriever` and {class}`~flexrag.retriever.FlexRetriever`. The relationship between these retrievers is shown in the following figure:
